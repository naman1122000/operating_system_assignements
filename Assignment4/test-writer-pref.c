...
int main(int argc, char *argv[])
{
	...
	index_count = 0;
	readerAcquireTime = malloc(sizeof(long)*read_num_threads*4);
	readerReleaseTime = malloc(sizeof(long)*read_num_threads*4);
	writerAcquireTime = malloc(sizeof(long)*write_num_threads*2);
	writerReleaseTime = malloc(sizeof(long)*write_num_threads*2);
	pthread_mutex_init(&spinlock, 0);

	int num_threads = 4*read_num_threads + 2*write_num_threads;
    ...
    for(int i=0;i<write_num_threads/2;i++)
	{
		int *arg = (int *)malloc((sizeof(int)));
		if (arg == NULL)
		{
	...
	for(int i=write_num_threads/2;i<write_num_threads;i++)
	{
		int *arg = (int *)malloc((sizeof(int)));
		if (arg == NULL)
		{
